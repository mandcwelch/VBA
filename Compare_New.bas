...
If Cells(i, 1).Interior.ColorIndex = 48 Then

    Cells(i, 1).EntireRow.Copy
    Cells(endDrop, 1).Insert
    Cells(i, 1) = ""
    endDrop = endDrop + 1
...
If Cells(i, x) <> w1.Cells(i, x) Then Cells(i, x).Interior.ColorIndex = 27
...
If Cells(i, x) <> w2.Cells(i, x) Then Cells(i, x).Interior.ColorIndex = 27
...
Range(Cells(1, 1), Cells(5000, 1)).SpecialCells(xlCellTypeBlanks).EntireRow.Delete

endRow = Cells(1, 1).End(xlDown).Row

Range(Cells(1, 1), Cells(endRow, 17)).Sort Key1:=Range("C2"), order1:=xlAscending, key2:=Range("D2"), order2:=xlAscending, _
Header:=xlYes
